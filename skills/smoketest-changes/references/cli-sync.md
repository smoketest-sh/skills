...
Use this reference before any Smoketest CLI interaction. The full changes workflow starts by checking CLI availability and authentication. Draft-only discovery can proceed without the CLI only when the user explicitly chooses that fallback; create/update/apply always requires the CLI.
...
```bash
node skills/smoketest-changes/scripts/apply-manifest.mjs .smoketest/changes/manifest.json --project "<project>"
```

Apply after explicit user approval:

```bash
node skills/smoketest-changes/scripts/apply-manifest.mjs .smoketest/changes/manifest.json --project "<project>" --apply
```
...
Run this gate before change analysis:
...
smoketest flows create \
  ...
  --description @.smoketest/changes/flows/pricing-annual-toggle.md \
  --device both \
  --json
```
...
```bash
smoketest flows edit "Billing settings" \
  --project "<project>" \
  --url "https://example.com/dashboard/billing" \
  --description @.smoketest/changes/flows/billing-settings.md \
  --device both \
  --json
```
...
- Report deleted or renamed routes as possible stale coverage; do not delete resources.
